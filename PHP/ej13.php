<?php 
    ...
    function resolverInyeccion($campo){
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['submit'])){
        ...
        $nombre = resolverInyeccion(procesarError('nombre', $errores));
        $apellidos = resolverInyeccion(procesarError('apellidos', $errores));
        $dni = resolverInyeccion(procesarError('dni', $errores));
        $fechaNacimiento = resolverInyeccion(procesarError('fecha-nacimiento', $errores));
        $nacionalidad = resolverInyeccion($_GET['nacionalidad']);
        $email = resolverInyeccion(procesarError('email', $errores));
        $clave = resolverInyeccion(procesarError('clave', $errores));
        $claveRepetida = resolverInyeccion(procesarError('clave-repetida', $errores));
        $sexo = resolverInyeccion(procesarError('sexo', $errores));
        $idiomaComunicacion = resolverInyeccion(procesarError('idioma-comunicacion', $errores));
        $tratamiento = resolverInyeccion(procesarError('tratamiento', $errores));
        if(isset($errores['preferencia'])){
            $preferencias = [];
        } else {
            if(isset($_GET['preferencia'])){
                $preferencias = array_map('htmlspecialchars', $_GET['preferencia']);
            } else {
                $preferencias = [];
            }
        }
        ...
        <div class="main">
            <main>
                <section class="registro-usuarios">
                    ...
                    <p>Bienvenido/a <?php echo $nombre ?>, siga instrucciones a continuación:</p>
